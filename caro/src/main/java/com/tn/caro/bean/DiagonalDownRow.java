...
public class DiagonalDownRow extends Row {
	public DiagonalDownRow(Cell cellBegin, Cell cellEnd) {
		super(Row.ROW_TYPE_DIAGONAL_DOWN, (byte)(Math.abs(cellBegin.getX()-cellEnd.getX()) + 1), cellBegin, cellEnd);
		allCheckedRow = new HashMap<Byte, List<CheckedRow>>();
	}

	@Override
	void setStepToRow(Step newStep) {
		allEmptyCell[Math.abs(newStep.getX() - cellBegin.getX())] = newStep;
	}
